...
const DrawNote = () => {
  ...
  const { createSpaceNote, updateSpaceNote, fetchSpaceNotes, notes } = useSpaces();
  ...
  useEffect(() => {
    if (!isEditMode || !editNoteId || !fabricCanvasRef.current) return;

    const loadNote = async () => {
      setIsLoading(true);
      let note = notes.find((n) => String(n.id) === String(editNoteId));
      if (!note) {
        try {
          await fetchSpaceNotes(
            localStorage.getItem('aigendaActiveWorkspaceId'),
            localStorage.getItem('aigendaActiveSpaceId')
          );
          note = notes.find((n) => String(n.id) === String(editNoteId));
        } catch (e) {
          console.error('Failed to fetch notes for edit:', e);
        }
      }
      if (note) {
        setNoteTitle(note.title || '');
        const canvas = fabricCanvasRef.current;
        if (note.drawingData && canvas) {
          try {
            isHistoryUpdate.current = true;
            const json = typeof note.drawingData === 'string' ? JSON.parse(note.drawingData) : note.drawingData;
    ...
    loadNote();
  }, [isEditMode, editNoteId, notes, fetchSpaceNotes]);
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <input
            ...
            style={{ border: '1px solid #e2e8f0', borderRadius: '8px', padding: '8px 10px' }}
          />
          ...
          <p style={{ color: '#a9a9a9', margin: 0, display: 'flex', alignItems: 'center', gap: '5px' }}>
            <span style={{ fontSize: '10px' }}>●</span> {isEditMode ? 'Edit canva note' : 'New canva note'}
          </p>
        </div>
        ...
        <button
          ...
          onClick={async () => {
            ...
            try {
              ...
              const multiplier = canvasWidth > 800 ? 800 / canvasWidth : 1;
              const dataUrl = canvas.toDataURL({ format: 'png', multiplier });
              const imageFile = await (await fetch(dataUrl)).blob();
              const workspaceId = localStorage.getItem('aigendaActiveWorkspaceId');
              const spaceId = localStorage.getItem('aigendaActiveSpaceId');
              const payload = {
                type: 'draw',
                title: noteTitle.trim(),
                content: 'Canvas note',
                drawingData: canvas.toJSON(),
                imageFile: imageFile || null,
                imageFileName: 'drawing.png',
              };
              if (isEditMode) {
                await updateSpaceNote(workspaceId, spaceId, editNoteId, payload);
              } else {
                await createSpaceNote(workspaceId, spaceId, payload);
              }
              navigate('/workspacenotes');
            } catch (err) {
              console.error('Error saving draw note:', err);
              alert('Failed to save drawing note. Please try again.');
            } finally {
              setIsSaving(false);
            }
          }}
